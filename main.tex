...
\subsection{Увеличение и сокращение потока}

Поток в остаточной сети предоставляет указания по добавлению потока к исходной транспортной сети. Если $f$ представляет собой поток в $G$, а $f^{'}$ представляет собой поток в соответствующей остаточной сети $G_f$ определим \textbf{увеличение} $f \nearrow f^{'}$ потока $f$ на $f^{'}$ как функцию, отображающую $V \times V$ на $\mathds{R}$ определенную следующим образом:
...
int main()
{
	...
	int flow = 0,  cost = 0;
	while (flow < k) {
		vector<int> id (n, 0); vector<int> d (n, INF);
		vector<int> q (n); vector<int> p (n);
